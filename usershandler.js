...
var engine = User.prototype = {
    ...
    processVoicemailNotification: function(body){
      ...
      // just for my test with predefined spam number
      if (body.from.phoneNumber == "+16502245476")
        useSample = true
      else
        useSample = false
      ...
      }else{ // no phone number => private call. Need to detect number from the voice message
          console.log("TBI")
      }
    },
            ...
            transcriptionist.transcribe(recordingUrl, function(err, transcript){
              if (err){
                ...
                if (wordArr.length > 10 && item.spam.reputation_level == 1){
                    thisUser.analyzeVoicemail(transcript, item)
    ...
    analyzeVoicemail: function (transcript, item){
      var thisUser = this
      content_analysis.classifyUrgency(transcript, function(err, result){
        item['status'] = result['status']
        item['confidence'] = result['confidence']
        // reply with SMS message to customer
        // remove this later when stop using samplePhoneNumber
        /*
        var canSendSMS = true
        for (var number of samplePhoneNumber){
            if (number == item.fromNumber){
              canSendSMS = false
              break
            }
        }
        ...
        */
        // use this when stop using samplePhoneNumber
        if (item.fromNumber != "Unknown"){
            ...
            for (agent of thisUser.settings.assigned_agents){
              for (var cat of agent.category){
                if (cat == result.category){
                  ...
                  console.log("Assigned " + cat + " case to " + agent.name)
...
function addFakeCustomersData(table){
  var data = [
    ...
    [3,	"Daniel","Rocha","+17242680235","landline"],
    [4,	"Martin","Barclay","+16504281183","landline"],
    [5,	"Mike","Mahony","4085942101","landline"],
    [6, "Paco","Vu","+16505130930", "mobile"],
...
///
/*
'5084330816': Vacation Scam
'9294749128': Telemarketer
'2025730012': Extortion
'2816488775': Phishing
'6196583462': Lucky Winner Scam
'6507291497': TollFree Pumping
'6502246623': Phone Survey
'6502958113': IRS Scam
*/
